...
function mervis_display_order_form_tab_content() {
    ...
    // ✅ حذف wp_enqueue_script فایل قدیمی برای جلوگیری از تداخل
    ...
    <style>
        ...
        .mervis-form-modern input[type="text"], .mervis-form-modern input[type="number"], .mervis-form-modern input[type="tel"], .mervis-form-modern select { width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px; font-size:14px; background:white; box-sizing:border-box; }
        ...
        .mervis-horizontal-options .mervis-option-label { display:inline-flex !important; align-items:center; gap:8px; cursor:pointer; background:white; padding:8px 18px; border-radius:50px; border:1.5px solid #e2e8f0; }
        ...
        /* ✅ استایل ضدگلوله برای مخفی کردن فیلدهای شرطی */
            ...
            <div class="mervis-form-modern">
                <form id="mervis-dynamic-order-form" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-grid-4cols">
                        ...
                        ?>
                            <!-- ✅ اصلاح فاصله قبل از style -->
                            <div class="<?php echo esc_attr($row_class . ' ' . $cond_class); ?>" <?php echo $cond_attrs; ?> <?php if ($is_conditional): ?>style="display: none !important;"<?php endif; ?>>
    ...
    (function($) {
        ...
        function parsePersianFloat(str) {
            if (!str) return NaN;
            return parseFloat(String(str).replace(/[۰-۹]/g, function(w) { return String.fromCharCode(w.charCodeAt(0) - 1728); }).replace(/[٠-٩]/g, function(w) { return String.fromCharCode(w.charCodeAt(0) - 1584); }).trim());
        }
        ...
        // ============================================
        // ✅ مدیریت فیلدهای شرطی (نسخه نهایی و قطعی)
        // ============================================
        window.applyConditionalLogic = function() {
            $('.mervis-conditional-field').each(function() {
                ...
                if (!parentKey || !targetValue) {
                    $row.addClass('mervis-visible').css('display', '').find('input, select, textarea').prop('disabled', false);
                    return;
                }
                ...
                if (isMatch) {
                    ...
                    $row.removeClass('mervis-visible').css('display', 'none !important').find('input, select, textarea').prop('disabled', true).val('');
        ...
        $(document).ready(function() {
            window.applyConditionalLogic();
            calculatePrice();
            setTimeout(window.applyConditionalLogic, 200);
            setTimeout(window.applyConditionalLogic, 600);
        });
